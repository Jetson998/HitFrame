...
const route = async (userInput, uploadedImages, skillId, references) => {
  const r = await fetch(`${BASE}/agent/route`, {
    method: 'POST',
    headers: H,
    body: JSON.stringify({ userInput, uploadedImages, skillId, references }),
  });
...
async function main() {
  ...
  check('A2 /templates 返回三模板', ids.join(',') === 'tpl_bg,tpl_model,tpl_poster', ids.join(','));
  const leaked =
    JSON.stringify(tpls).includes('promptTemplate') ||
    JSON.stringify(tpls).includes('prompt_template');
  ...
  const cases = [
    [
      '商品换大理石背景，1张，1:1',
      { uploadedImages: ['x.jpg'] },
      (p) =>
        p.mode === 'template' &&
        p.templateId === 'tpl_bg' &&
        p.params.candidateCount === 1 &&
        p.params.ratio === '1:1',
    ],
    ...
    [
      ...
      {},
      (p) =>
        p.templateId === 'tpl_model' && p.params.ratio === '3:4' && p.params.candidateCount === 1,
    ],
    [
      '小红书封面，标题夏日新品，9:16',
      {},
      (p) => p.templateId === 'tpl_poster' && p.params.ratio === '9:16',
    ],
  ...
  for (let i = 0; i < cases.length; i++) {
    ...
    check(
      `B${i + 1} route: "${input.slice(0, 20)}"`,
      assertFn(plan),
      JSON.stringify(plan.params ?? {}) +
        (plan.templateId ? ` tpl=${plan.templateId}` : ` mode=${plan.mode}`),
    );
  }

  const explicitSkill = await route(
    '根据摘要生成一张文章配图',
    ['ref.jpg'],
    'skill_article_illustration',
    [{ assetId: 'ref.jpg', role: 'style', fidelity: 'auto' }],
  );
  check(
    'B9 显式 Skill + 参考图走 i2i',
    explicitSkill.skillId === 'skill_article_illustration' && explicitSkill.mode === 'i2i',
    `skill=${explicitSkill.skillId}; mode=${explicitSkill.mode}`,
  );

  const missingRole = await route('制作商品海报', ['style.jpg'], 'skill_ecommerce_poster', [
    { assetId: 'style.jpg', role: 'style', fidelity: 'auto' },
  ]);
  check(
    'B10 必填参考图按角色判断',
    missingRole.missingSlots?.includes('需上传商品主体'),
    JSON.stringify(missingRole.missingSlots ?? []),
  );

  // route 无副作用：所有分析调用后余额不变、无新 Run
  const runCountBefore = psql('SELECT count(*) FROM generation_runs');
  check(
    'B11 route 无副作用（余额与 Run 不变）',
    bal() === bal0 && psql('SELECT count(*) FROM generation_runs') === runCountBefore,
    `${bal()} vs ${bal0}; runs=${runCountBefore}`,
  );
  ...
  psql(
    `INSERT INTO assets (id, tenant_id, type, url, name, meta) VALUES ('${srcAsset}','default','source','http://x/${MARK}.png','${MARK}-src','{"storageKey":"uploads/${MARK}.png"}')`,
  );
  // 造实际文件供 loadSlots 读取
  execSync(
    `mkdir -p storage/uploads && printf '\\x89PNG\\r\\n\\x1a\\n' > storage/uploads/${MARK}.png`,
    { shell: '/bin/bash' },
  );
  ...
  const modelRun = await genTemplate('tpl_model', {
    modelType: '时尚模特',
    pose: '自然站姿',
    scene: '纯色背景',
  });
  check('C2 tpl_model 生成成功', modelRun.status === 'succeeded', `status=${modelRun.status}`);
  const posterRun = await genTemplate('tpl_poster', {
    title: '夏日新品',
    sellingPoint: '买二送一',
    style: '简约现代',
  });
  ...
  const bgAssetParams = psql(
    `SELECT gen_params FROM assets WHERE source_job_id='${bgRun.jobs[0].jobId}'`,
  );
  check(
    'C4 Asset genParams 含 templateId/prompt/vars',
    bgAssetParams.includes('tpl_bg') &&
      bgAssetParams.includes('大理石') &&
      bgAssetParams.includes('prompt'),
    '',
  );
  ...
  const keys = psql(
    `SELECT meta->>'storageKey' FROM assets WHERE name LIKE '%${MARK}%' OR gen_params->>'runId' IN (SELECT id FROM generation_runs WHERE idempotency_key LIKE '%')`,
  )
    .split('\n')
    .filter(Boolean);
  ...
  for (const rid of runIds) {
    psql(
      `DELETE FROM assets WHERE source_job_id IN (SELECT id FROM generation_jobs WHERE run_id='${rid}')`,
    );
  ...
  psql(
    `UPDATE tenants SET points_balance=(SELECT COALESCE(SUM(amount),0) FROM credit_transactions WHERE tenant_id='default') WHERE id='default'`,
  );
  const [b, s] = psql(
